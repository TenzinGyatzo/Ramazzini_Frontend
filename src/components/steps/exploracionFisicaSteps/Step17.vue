...
onMounted(() => {
    ...
    if (formDataExploracionFisica.caderaPregunta !== undefined && formDataExploracionFisica.caderaPregunta !== null && formDataExploracionFisica.caderaPregunta !== '') {
        caderaPregunta.value = formDataExploracionFisica.caderaPregunta;
        cadera.value = formDataExploracionFisica.cadera || '';
    } else if (formDataExploracionFisica.cadera !== undefined && formDataExploracionFisica.cadera !== null && formDataExploracionFisica.cadera !== '') {
        cadera.value = formDataExploracionFisica.cadera;
        caderaPregunta.value = formDataExploracionFisica.caderaPregunta || (formDataExploracionFisica.cadera !== 'Sin hallazgos' ? 'Si' : 'No');
    } else if (documentos.currentDocument) {
        caderaPregunta.value = documentos.currentDocument.caderaPregunta || 'No';
        cadera.value = documentos.currentDocument.cadera || '';
    } else {
        caderaPregunta.value = formDataExploracionFisica.caderaPregunta || 'No';
        cadera.value = formDataExploracionFisica.cadera || '';
    }
});

onUnmounted(() => {
    if (!formDataExploracionFisica.caderaPregunta) {
        formDataExploracionFisica.caderaPregunta = caderaPregunta.value;
    }
...
watch(caderaPregunta, (newValue) => {
    formDataExploracionFisica.caderaPregunta = newValue;
});
...
watch(caderaPregunta, async (newValue, oldValue) => {
    // Solo cambios del usuario (no montaje / remount).
    if (oldValue === undefined || oldValue === newValue) return;
    if (newValue === 'No') {
        formDataExploracionFisica.cadera = 'Sin hallazgos';
        cadera.value = 'Sin hallazgos';
    }
    if (newValue === 'Si') {
        const cur = (cadera.value || formDataExploracionFisica.cadera || '').trim();
        if (cur === 'Sin hallazgos') {
            cadera.value = '';
            formDataExploracionFisica.cadera = '';
        } else {
            formDataExploracionFisica.cadera = cadera.value || formDataExploracionFisica.cadera;
        }
        await nextTick();
    ...
    <div>
        <template v-if="variant === 'compact'">
            <HallazgoSiNoChips
                ...
                v-model="caderaPregunta"
                v-model:especificar="formDataExploracionFisica.cadera"
                placeholder="Describa los hallazgos encontrados..."
            />
        </template>
        <template v-else>
        <h1 class="text-2xl font-bold mb-4 text-gray-900">Extremidades Inferiores</h1>
        <h2 class="text-lg font-semibold mb-4 text-gray-700">CADERA</h2>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                >
                    ...
                    <div 
                        :class="[
                            ...
                            caderaPregunta === 'No' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'
                        ]"
                    >
                    ...
                    <span 
                        :class="[
                            ...
                            caderaPregunta === 'No' ? 'text-emerald-700' : 'text-gray-700'
                        ]"
                    >
                        No
                    </span>
                    <div 
                        v-if="caderaPregunta === 'No'"
                ...
                >
                    ...
                    <div 
                        :class="[
                            'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
                            caderaPregunta === 'Si' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'
                        ]"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <span 
                        :class="[
                            'text-base font-semibold transition-colors duration-200',
                            caderaPregunta === 'Si' ? 'text-emerald-700' : 'text-gray-700'
                        ]"
                    >
                        Sí
                    </span>
                    <div 
                        v-if="caderaPregunta === 'Si'"
                        class="absolute top-2 right-2 w-4 h-4 bg-emerald-600 rounded-full flex items-center justify-center"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-2.5 w-2.5 text-white" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
        ...
        >
            <div v-if="caderaPregunta === 'Si'" class="mt-6">
