...
onMounted(() => {
  const nombre = (formDataAptitud.evaluacionAdicional2 || '').trim();
  const resultados = (formDataAptitud.resultadosEvaluacionAdicional2 || '').trim();
  const fechaRaw = formDataAptitud.fechaEvaluacionAdicional2;

  if (fechaRaw) {
    formDataAptitud.fechaEvaluacionAdicional2 = formatDateYYYYMMDD(fechaRaw);
  } else if (!isCompact.value) {
    // V1 fullscreen: al visitar el paso, fecha = hoy (comportamiento legacy).
    formDataAptitud.fechaEvaluacionAdicional2 = today;
...
<template>
  <div>
    ...
    >EVALUACIÓN ADICIONAL 2</h1>
    <h2
      v-else
      class="text-sm font-semibold mb-2 text-gray-800"
    >EVALUACIÓN ADICIONAL 2</h2>
    ...
    <div :class="isCompact ? 'space-y-2' : 'space-y-6'">
      <div :class="fieldClass">
        ...
        <input
          type="text"
          name="nombreEvaluacion2"
          data-skip-validation
          :class="inputClass"
          v-model="formDataAptitud.evaluacionAdicional2"
          :list="'evaluaciones-sugerencias-2'"
          placeholder="Selecciona o escribe"
        />
        <datalist :id="'evaluaciones-sugerencias-2'">
