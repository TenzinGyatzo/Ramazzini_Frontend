...
onMounted(() => {
  const nombre = (formDataAptitud.evaluacionAdicional1 || '').trim();
  const resultados = (formDataAptitud.resultadosEvaluacionAdicional1 || '').trim();
  const fechaRaw = formDataAptitud.fechaEvaluacionAdicional1;

  if (fechaRaw) {
    formDataAptitud.fechaEvaluacionAdicional1 = formatDateYYYYMMDD(fechaRaw);
  } else if (!isCompact.value) {
    // V1 fullscreen: al visitar el paso, fecha = hoy (comportamiento legacy).
    formDataAptitud.fechaEvaluacionAdicional1 = today;
...
<template>
  <div>
    ...
    >EVALUACIÓN ADICIONAL 1</h1>
    <h2
      v-else
      class="text-sm font-semibold mb-2 text-gray-800"
    >EVALUACIÓN ADICIONAL 1</h2>
    ...
    <div :class="isCompact ? 'space-y-2' : 'space-y-6'">
      <div :class="fieldClass">
        ...
        <input
          type="text"
          name="nombreEvaluacion1"
          data-skip-validation
          :class="inputClass"
          v-model="formDataAptitud.evaluacionAdicional1"
          :list="'evaluaciones-sugerencias-1'"
          placeholder="Selecciona o escribe"
        />
        <datalist :id="'evaluaciones-sugerencias-1'">
      ...
      <div :class="fieldClass">
        ...
        <input
          type="date"
          name="fechaEvaluacion1"
          :class="inputClass"
          v-model="formDataAptitud.fechaEvaluacionAdicional1"
        />
      </div>
      ...
      <div :class="isCompact ? 'mb-1' : 'mb-6'">
        ...
        <input
          type="text"
          name="resultadosEvaluacion1"
          data-skip-validation
          :class="inputClass"
          v-model="formDataAptitud.resultadosEvaluacionAdicional1"
