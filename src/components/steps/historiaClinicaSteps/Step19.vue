...
// Watch para establecer 'Negado' cuando accidentes sea 'No' y enfocar input cuando sea 'Si'
watch(accidentes, async (newValue) => {
    if (newValue === 'No') {
        formDataHistoriaClinica.accidentesEspecificar = 'Negado';
    }
    if (newValue === 'Si') {
    ...
    <div>
        <template v-if="variant === 'compact'">
            <AntecedenteSiNoChips
                label="ACCIDENTES"
                question="¿Ha experimentado el trabajador algún accidente significativo?"
                v-model="accidentes"
                v-model:especificar="formDataHistoriaClinica.accidentesEspecificar"
                placeholder="Especifique"
            />
        </template>
        <template v-else>
        <h1 class="text-2xl font-bold mb-4 text-gray-900">Antecedentes Patológicos</h1>
        <h2 class="text-lg font-semibold mb-4 text-gray-700">ACCIDENTES</h2>
        <div class="mb-8">
            <p class="text-lg font-medium mb-4 text-gray-800">¿Ha experimentado el trabajador algún accidente significativo?</p>
            <div class="grid grid-cols-2 gap-3">
                ...
                >
                    <input type="radio" value="No" v-model="accidentes" class="sr-only" />
                    <div 
                        :class="[
                            'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
                            accidentes === 'No' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'
                        ]"
                    >
                    ...
                    <span 
                        :class="[
                            ...
                            accidentes === 'No' ? 'text-emerald-700' : 'text-gray-700'
                        ]"
                    >No</span>
                    <div v-if="accidentes === 'No'" class="absolute top-2 right-2 w-4 h-4 bg-emerald-600 rounded-full flex items-center justify-center">
                ...
                >
                    <input type="radio" value="Si" v-model="accidentes" class="sr-only" />
                    <div 
                        :class="[
                            'w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors duration-200',
                            accidentes === 'Si' ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600'
                        ]"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <span 
                        :class="[
                            'text-base font-semibold transition-colors duration-200',
                            accidentes === 'Si' ? 'text-emerald-700' : 'text-gray-700'
                        ]"
                    >Sí</span>
                    <div v-if="accidentes === 'Si'" class="absolute top-2 right-2 w-4 h-4 bg-emerald-600 rounded-full flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-2.5 w-2.5 text-white" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                        </svg>
                    </div>
        ...
        >
            <div v-if="accidentes === 'Si'" class="mt-6">
