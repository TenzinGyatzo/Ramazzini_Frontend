...
onMounted(() => {
  const nombre = (formDataAptitud.evaluacionAdicional4 || '').trim();
  const resultados = (formDataAptitud.resultadosEvaluacionAdicional4 || '').trim();
  const fechaRaw = formDataAptitud.fechaEvaluacionAdicional4;

  if (fechaRaw) {
    formDataAptitud.fechaEvaluacionAdicional4 = formatDateYYYYMMDD(fechaRaw);
  } else if (!isCompact.value) {
    // V1 fullscreen: al visitar el paso, fecha = hoy (comportamiento legacy).
    formDataAptitud.fechaEvaluacionAdicional4 = today;
...
<template>
  <div>
    ...
    >EVALUACIÓN ADICIONAL 4</h1>
    <h2
      v-else
      class="text-sm font-semibold mb-2 text-gray-800"
    >EVALUACIÓN ADICIONAL 4</h2>
    ...
    <div :class="isCompact ? 'space-y-2' : 'space-y-6'">
      <div :class="fieldClass">
        ...
        <input
          type="text"
          name="nombreEvaluacion4"
          data-skip-validation
          :class="inputClass"
          v-model="formDataAptitud.evaluacionAdicional4"
          :list="'evaluaciones-sugerencias-4'"
          placeholder="Selecciona o escribe"
        />
        <datalist :id="'evaluaciones-sugerencias-4'">
      ...
      <div :class="fieldClass">
        ...
        <input
          type="date"
          name="fechaEvaluacion4"
          :class="inputClass"
          v-model="formDataAptitud.fechaEvaluacionAdicional4"
        />
      </div>
      ...
      <div :class="isCompact ? 'mb-1' : 'mb-6'">
        ...
        <input
          type="text"
          name="resultadosEvaluacion4"
          data-skip-validation
          :class="inputClass"
          v-model="formDataAptitud.resultadosEvaluacionAdicional4"
