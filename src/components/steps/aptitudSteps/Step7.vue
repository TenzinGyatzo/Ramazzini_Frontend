...
onMounted(() => {
  const nombre = (formDataAptitud.evaluacionAdicional6 || '').trim();
  const resultados = (formDataAptitud.resultadosEvaluacionAdicional6 || '').trim();
  const fechaRaw = formDataAptitud.fechaEvaluacionAdicional6;

  if (fechaRaw) {
    formDataAptitud.fechaEvaluacionAdicional6 = formatDateYYYYMMDD(fechaRaw);
  } else if (!isCompact.value) {
    // V1 fullscreen: al visitar el paso, fecha = hoy (comportamiento legacy).
    formDataAptitud.fechaEvaluacionAdicional6 = today;
...
<template>
  <div>
    ...
    >EVALUACIÓN ADICIONAL 6</h1>
    <h2
      v-else
      class="text-sm font-semibold mb-2 text-gray-800"
    >EVALUACIÓN ADICIONAL 6</h2>
    ...
    <div :class="isCompact ? 'space-y-2' : 'space-y-6'">
      <div :class="fieldClass">
        ...
        <input
          type="text"
          name="nombreEvaluacion6"
          data-skip-validation
          :class="inputClass"
          v-model="formDataAptitud.evaluacionAdicional6"
          :list="'evaluaciones-sugerencias-6'"
          placeholder="Selecciona o escribe"
        />
        <datalist :id="'evaluaciones-sugerencias-6'">
      ...
      <div :class="fieldClass">
        ...
        <input
          type="date"
          name="fechaEvaluacion6"
          :class="inputClass"
          v-model="formDataAptitud.fechaEvaluacionAdicional6"
        />
      </div>
      ...
      <div :class="isCompact ? 'mb-1' : 'mb-6'">
        ...
        <input
          type="text"
          name="resultadosEvaluacion6"
          data-skip-validation
          :class="inputClass"
          v-model="formDataAptitud.resultadosEvaluacionAdicional6"
